...
// ============================================================
// БЛОК 1: КОНФІГУРАЦІЯ ПРОЄКТУ
// ============================================================

// ---- 1.1 Налаштування Telegram Bot ----
...
// ---- 1.4 Налаштування NTP (синхронізація часу) ----
...
// ---- 1.5 Список Wi-Fi мереж для підключення ----
...
// ============================================================
// БЛОК 2: ПІДКЛЮЧЕННЯ БІБЛІОТЕК ТА ГЛОБАЛЬНІ ЗМІННІ
// ============================================================
...
bool inMonthMenu = false;                // Прапорець режиму вибору місяця

// ---- 2.4 Змінні для відліку часу ----
...
// ---- 2.5 Структура та масив для історії подій ----
struct EventLog
{
...
int eventCount = 0; // Поточна кількість подій

// ---- 2.6 Змінні для роботи з часом ----
time_t currentTime = 0;
struct tm timeinfo;

// ============================================================
// БЛОК 3: ФУНКЦІЇ РОБОТИ З АПАРАТНИМ ЗАБЕЗПЕЧЕННЯМ
// ============================================================
...
// ============================================================
// БЛОК 4: ФУНКЦІЇ ФОРМАТУВАННЯ ТА ДОПОМІЖНІ
// ============================================================
...
// ============================================================
// БЛОК 5: ФУНКЦІЇ РОБОТИ З ДАТАМИ ТА МІСЯЦЯМИ
// ============================================================
...
// ============================================================
// БЛОК 6: РОБОТА З ФАЙЛОВОЮ СИСТЕМОЮ (LittleFS)
// ============================================================
...
// ============================================================
// БЛОК 7: ПІДКЛЮЧЕННЯ ДО МЕРЕЖІ ТА СИНХРОНІЗАЦІЯ ЧАСУ
// ============================================================
...
// ============================================================
// БЛОК 8: ТЕЛЕГРАМ МЕНЮ ТА ІНТЕРФЕЙС
// ============================================================
...
// ============================================================
// БЛОК 9: ОБРОБНИК ТЕЛЕГРАМ ПОВІДОМЛЕНЬ
// ============================================================
...
void newMsg(FB_msg &msg)
{
  ...
  // ---- 9.2 Режим вибору місяця (блоковані всі команди крім вибору та Назад) ----
  if (inMonthMenu)
  {
    ...
    if (isValidMonth && selectedMonth >= 1 && selectedMonth <= 12)
    {
      ...
      // Формуємо відповідь зі статистикою
      ...
      // Додаємо підказку для повернення
  ...
  // ---- 9.3 Основний режим (обробляємо всі команди) ----
...
// ============================================================
// БЛОК 10: ІНІЦІАЛІЗАЦІЯ (SETUP)
// ============================================================

void setup()
{
  // ---- 10.1 Ініціалізація Serial ----
  Serial.begin(115200);
  delay(3000);
  Serial.println("\n🚀 Запуск WEMOS...");
  delay(3000);

  // ---- 10.2 Ініціалізація файлової системи ----
  if (!LittleFS.begin())
  {
  ...
  // ---- 10.3 Налаштування пінів ----
  pinMode(POWER_PIN, INPUT);
  pinMode(relePin, OUTPUT);
  digitalWrite(relePin, HIGH);

  // ---- 10.4 Завантаження збережених даних ----
  loadData();

  // ---- 10.5 Підключення до Wi-Fi ----
  rowsWifiList = (sizeof(wifiList) / sizeof(wifiList[0]));
  connectWiFi();

  // ---- 10.6 Синхронізація часу ----
  syncTime();

  // ---- 10.7 Визначення початкового стану ПК ----
  float voltage = readVoltage();
  pcIsOn = (voltage > VOLTAGE_ON);
  prevPcState = pcIsOn;

  // ---- 10.8 Додаємо першу подію в історію ----
  ...
  // ---- 10.9 Ініціалізація Telegram бота ----
  bot.attach(newMsg);
  delay(1000);

  // ---- 10.10 Відправка привітального повідомлення ----
  Serial.println("\n✅ WEMOS: увімкнувся!");
...
// ============================================================
// БЛОК 11: ГОЛОВНИЙ ЦИКЛ (LOOP)
// ============================================================

void loop()
{
  // ---- 11.1 Контроль Wi-Fi підключення ----
  if (WiFi.status() != WL_CONNECTED)
  {
  ...
  // ---- 11.2 Обробка повідомлень Telegram ----
  bot.tick();

  // ---- 11.3 Оновлення часу з NTP (кожну годину) ----
  ...
  // ---- 11.5 Вимірювання напруги та визначення стану ПК ----
  float voltage = readVoltage();
  ...
  // ---- 11.6 Відправка сповіщень при зміні стану ПК та запис в історію ----
  ...
  // ---- 11.7 Автоматичне збереження даних (кожну секунду) ----
  ...
  // ---- 11.8 Планове перезавантаження кожні 24 години ----
